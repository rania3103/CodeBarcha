...
function SignUp() {
    ...
    const handleSignUp = async() =>{
        try {
            await axios.post('http://localhost:3000/api/auth/register', {email, githubUserName, password});
            navigate("signin")
        } catch (error) {
            console.error(error);
        }
    ...
    return (
        <div className="flex items-center justify-center h-screen bg-indigo-800">
          <div className="bg-slate-100 p-8 rounded shadow-md w-96 border-2 border-indigo-500">
            ...
            <button className="bg-indigo-500 text-white px-5 py-1 font-bold rounded ml-24 hover:bg-indigo-400" onClick={handleSignUp}>Login</button>
